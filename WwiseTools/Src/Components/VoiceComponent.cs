...
namespace WwiseTools.Components
{
    public class VoiceComponent : ComponentBase
    {
        ...
        public async Task SetAuxilaryBus0Async(string busName, float volume = 0, uint lowPass = 0, uint highPass = 0)
        {
            ...
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_UserAuxSend0(await WwiseUtility.Instance.GetWwiseObjectByNameAsync($"AuxBus:{busName}")));
        }
        public async Task SetAuxilaryBus1Async(string busName, float volume = 0, uint lowPass = 0, uint highPass = 0)
        {
            ...
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_UserAuxSend1(await WwiseUtility.Instance.GetWwiseObjectByNameAsync($"AuxBus:{busName}")));
        }
        ...
        public async Task SetAuxilaryBus2Async(string busName, float volume = 0, uint lowPass = 0, uint highPass = 0)
        {
            ...
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_UserAuxSend2(await WwiseUtility.Instance.GetWwiseObjectByNameAsync($"AuxBus:{busName}")));
        }
        public async Task SetAuxilaryBus3Async(string busName, float volume = 0, uint lowPass = 0, uint highPass = 0)
        {
            ...
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_UserAuxSend3(await WwiseUtility.Instance.GetWwiseObjectByNameAsync($"AuxBus:{busName}")));
        }
        ...
        public async Task SetEarlyReflectionsAsync(string busName, float volume = 0)
        {
            ...
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_ReflectionsAuxSend(await WwiseUtility.Instance.GetWwiseObjectByNameAsync($"AuxBus:{busName}")));
        }
        ...
        public async Task SetOutputBusAsync(string busName, float volume = 0, uint lowPass = 0, uint highPass = 0)
        {
            ...
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_OutputBus(await WwiseUtility.Instance.GetWwiseObjectByNameAsync($"Bus:{busName}")));
        }
        ...
        public async Task SetOutputBusAsync(WwiseObject bus, float volume = 0, uint lowPass = 0, uint highPass = 0)
        {
            ...
            await WwiseUtility.Instance.SetObjectPropertyAsync(WwiseObject, WwiseProperty.Prop_OutputBusLowpass(lowPass));
            await WwiseUtility.Instance.SetObjectPropertyAsync(WwiseObject, WwiseProperty.Prop_OutputBusHighpass(highPass));
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_OutputBus(bus));
        }
        ...
        public async Task SetConversionAsync(string conversionName)
        {
            ...
            await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_Conversion(await WwiseUtility.Instance.GetWwiseObjectByNameAsync($"Conversion:{conversionName}")));
        }
        ...
        public async Task SetAttenuationAsync(string attenuationName)
        {
            ...
            if (att == null)
            {
                att = await WwiseUtility.Instance.CreateObjectAsync(attenuationName, WwiseObject.ObjectType.Attenuation, @"\Attenuations\Default Work Unit");
            }
            if (att == null) return;
            await WwiseUtility.Instance.SetObjectPropertyAsync(WwiseObject, WwiseProperty.Prop_OverridePositioning(true));
            await WwiseUtility.Instance.SetObjectPropertyAsync(WwiseObject, WwiseProperty.Prop_ListenerRelativeRouting(true));
            if (!String.IsNullOrEmpty(attenuationName))
            {
                ...
                await WwiseUtility.Instance.SetObjectReferenceAsync(WwiseObject, WwiseReference.Ref_Attenuation(
                    att));
            }
        }
